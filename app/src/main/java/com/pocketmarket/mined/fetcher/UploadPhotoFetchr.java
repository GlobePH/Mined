...
import com.pocketmarket.mined.dto.GlobeOwnerShipDTO;
import com.pocketmarket.mined.dto.GlobeUserDTO;
    ...
    public UploadPhotoDTO fetchItems(String sUrl, String avatar, String accessToken, String fileName) {
        ...
        try {
            ...
            if (!jsonObject.get("uploadedForm").isJsonNull()){
                ...
                JsonObject userObject = uploadedFormObject.get("user").getAsJsonObject();

                String salutation = userObject.get("salutation").getAsString();
                String gender = userObject.get("gender").getAsString();

                GlobeUserDTO user = new GlobeUserDTO();
                user.setId(1);
                user.setSalutation(salutation);
                user.setGender(gender);

                uploadedFormDTO.setUser(user);

                String birthDate = uploadedFormObject.get("birthDate").getAsString();
                uploadedFormDTO.setBirthDate(birthDate);

                JsonObject ownerShipObject = uploadedFormObject.get("ownerShip").getAsJsonObject();
                String homeOwnerShip = ownerShipObject.get("homeOwnerShip").getAsString();
                String homeOwnerShipYears = ownerShipObject.get("homeOwnerShipYears").getAsString();
                String homeOwnerShipSalary = ownerShipObject.get("homeOwnerShipSalary").getAsString();

                GlobeOwnerShipDTO ownerShip = new GlobeOwnerShipDTO();
                ownerShip.setId(1);
                ownerShip.setHomeOwnerShip(homeOwnerShip);
                ownerShip.setHomeOwnerShipYears(homeOwnerShipYears);
                ownerShip.setHomeOwnerShipSalary(homeOwnerShipSalary);

                uploadedFormDTO.setOwnerShip(ownerShip);
